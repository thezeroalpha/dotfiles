...
PrevTabCount = 0
Wf_braveWindowTitle = hs.window.filter
  .new({ "Brave Browser" })
  :setOverrideFilter({ allowRoles = "AXStandardWindow" })
  :subscribe(hs.window.filter.windowTitleChanged, function()
    local success, tabCount = hs.osascript.applescript('tell application "Brave Browser" to count tab in first window')
    if not success then
      return
    end
    local threshold = 9
    if (tabCount > threshold and PrevTabCount <= threshold) or (tabCount <= threshold and PrevTabCount > threshold) then
